...
# Step 2: Move to the OU which is named with the month of 2 months later
    $OU = 'OU=Disabled,OU=Accounts,OU=PROD,DC=caanz,DC=com'
    $OUPrefix = 'OU=To Be Deleted -'
    switch((Get-Date).AddMonths(2).Month)
    {
...
# Remove all groups. Domain Users group will not be deleted by default.
    ...
    {
        Get-ADGroup $Group | Remove-ADGroupMember -Members $Name -Confirm:$False
    }
    Write-Host "Removed the group membership of account $($objAccount.Name)" `
        -ForegroundColor Green

# Step 4: Remove the manager in Orgnization tab
    $objManager = Get-ADUser $objAccount.Manager
    Set-ADUser -Identity $Name -Manager $Null
    Write-Host "Removed the manager $($objManager.Name) from $($objAccount.Name)"
}
        ...
        if( $FlagNewFolder -eq $True )
        {
            ...
            try
            {
                ...
                switch( $($objAccount.HomeDirectory.Substring(2, 3)) )
                {
                    ...
                    'NZD' {
                        Move-Folder $HomedirNZD $DestBase $Name $Name `
                            -ErrorAction Stop
                    }
                }
...
function Forward-Lync
{
    ...
    Write-Host "Connecting to Lync at $ConnectionUri" -ForegroundColor Cyan
    try
    {
        $Session = New-PSSession -ConnectionUri $ConnectionUri -Credential $Cred `
            -ErrorAction Stop
        Import-PSSession $Session -ErrorAction Stop
        Write-Host "Connected to Lync." -ForegroundColor Cyan
...
Please RDP to ECP-UC-V01 and start an elevated cmd.exe. Paste and execute.`n"
...
1. AD account operations:
    Disable AD account
    Move to Monthly Disabled OU (2 months)
...
while($True)
{
    ...
    try
    {
        switch($Option)
        {
            ...
            5 { Forward-Lync }
            6 { Normal-Depart $objAccount.SAMAccountName }
            default {
                Write-Host "Input must be a number from 0 to 6" `
                    -ForegroundColor Cyan
            }
        }
    }
    catch
    {
        ...
        Write-Host "$FailedItem --- $ErrorMsg" -ForegroundColor Red
    }
    Write-Host $Menu -ForegroundColor Cyan
}
